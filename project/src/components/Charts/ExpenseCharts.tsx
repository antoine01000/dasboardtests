...
// Importation du composant SelectedMonthDetails (inutile dans cette version)
import SelectedMonthDetails from "../Charts/SelectedMonthDetails";
...
const ExpenseCharts: React.FC = () => {
  ...
  // Calcul des données pour le graphique en barre
  const barData = {
    labels: months,
  ...
  // Nouvelle section : Dépenses par Personne pour le mois sélectionné
  const renderExpensesByPerson = () => {
    // Filtrer les dépenses pour le mois sélectionné
    const filteredByMonth = expenses.filter(e => e.month === selectedMonth);
    return (
      <section className="bg-white p-6 rounded-lg shadow-lg mb-6">
        <h2 className="text-2xl font-bold mb-4">Dépenses par Personne pour {selectedMonth}</h2>
                        ...
                        return (
                          <tr key={expense.id}>
                            ...
                            <td className="border-b p-1">
                              ...
                              <button 
                                onClick={() => { if(confirm('Supprimer cette dépense ?')) deleteExpense(expense.id); }}
  ...
  // Composant MonthlyTable existant (inchangé)
                ...
                <table className="w-full border-collapse">
                    ...
                    <tr>
                      ...
                      {months.map(month => {
                        ...
                        return (
                          <th
                            key={month}
                            className={`sticky top-0 z-10 bg-white min-w-[300px] p-4 border-b ${hoveredMonth === month ? 'bg-orange-50' : ''}`}
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between text-sm">
                              ...
                              {variation !== 0 && (
                                <div className={`flex items-center gap-1 ${variation > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  ...
                  <tbody>
                    {orderedCategories.map(category => (
                      <tr
                        key={category.id}
                        className={`group border-b hover:bg-gray-50 transition-colors ${draggedCategory?.id === category.id ? 'opacity-50' : ''}`}
                        draggable
  ...
  return (
    <section className="bg-white p-6 rounded-lg shadow-lg mb-6">
      ...
      <div className="mb-8">
        ...
        {/* Remplacement du tableau de détail par le nouveau tableau "Dépenses par Personne" pour le mois sélectionné */}
        {selectedMonth && renderExpensesByPerson()}
      </div>
